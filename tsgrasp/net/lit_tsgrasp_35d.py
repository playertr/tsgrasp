...
################################## PyTorch definition ##########################
class TSGraspNet35D(TSGraspSuper):
...
def multi_pointcloud_to_3d_coords(pcs):
    ...
    B, T, N, D = pcs.shape
    batch_coords = torch.arange(B*T, dtype=pcs.dtype, device=pcs.device).repeat_interleave(N)
    coords = torch.column_stack([batch_coords, pcs.reshape(-1, D)])
...
class LitTSGraspNet35D(pl.LightningModule):
    def __init__(self, model_cfg : DictConfig, training_cfg : DictConfig):
        super().__init__()
        self.save_hyperparameters()
        self.model = TSGraspNet35D(model_cfg)
    ...
    def forward(self, positions: torch.Tensor) -> Tuple[torch.Tensor, torch.TensorType, torch.Tensor, torch.Tensor]:
        ...
        voxelized_pos = discretize(positions, grid_size=self.model.grid_size) # B, 10, N_PTS, 3
        # stack up so that time is squished into batch dimension.
        coords = multi_pointcloud_to_3d_coords(voxelized_pos)
        ...
        # This reshape depends on the batch dimension coming first.
